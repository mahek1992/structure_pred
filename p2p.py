...
def get_device():
    if torch.cuda.is_available():
        try:
            ...
            print(f"CUDA initialized successfully. Using GPU: {device_name}")
            return torch.device("cuda"), "GPU"
        except Exception as e:
            print(f"Unable to initialize CUDA: {str(e)}")
...
if device.type == 'cuda':
    try:
        ...
        print(f"Error querying GPU memory: {str(e)}")
class ProteinDataset(Dataset):
   
    def __init__(self, df, max_antibody_sequence_length, max_antigen_sequence_length, save_dir='/mnt/storage-HDD01/1.scrach/immuno-mahek/p2p/graph_data'):
    ...
    def pdb_to_graph(self, pdb_file, max_sequence_length, graph_type):
        ...
        print(pdb_file)
        try:
            structure = pdbparser.get_structure(pdb_file, os.path.join(os.getcwd(), graph_type, pdb_file+'.pdb'))
            ...
            ppdb = PandasPdb()
            ppdb.read_pdb(os.path.join(os.getcwd(), graph_type, pdb_file+'.pdb'))
...
com_model_cp = torch.load('/mnt/storage-HDD01/1.scrach/immuno-mahek/p2p/model_weights.pth', map_location=device)
com_model_epoch = com_model_cp['epoch']
com_model.load_state_dict(com_model_cp['model_state_dict'])
files_to_process = [f for f in os.listdir("files") if f.endswith('.pdb')]

for file in files_to_process:
    # Move the file to antibodies folder
    shutil.move(os.path.join("files", file), os.path.join("antibodies", file))

    # List files in the directories
    antibody_files = [f for f in os.listdir("antibodies") if f.endswith('.pdb')]
    antigen_files = [f for f in os.listdir("antigens") if f.endswith('.pdb')]

    # Ensure exactly one file in each directory
    if len(antibody_files) != 1 or len(antigen_files) != 1:
        raise ValueError("Error: Please ensure exactly one .pdb file in each directory.")

    # Extract names without extension
    antibody_name = os.path.splitext(antibody_files[0])[0]
    antigen_name = os.path.splitext(antigen_files[0])[0]
    ...
    print(f"Processing: {antibody_name}")
    ...
    # Added print statement to show which device is being used
    print(f"Processing {antibody_name} using {device_name}")

    with torch.no_grad():
        for input in test_loader:
            # Modified to use the correct device
            ...
            binding_affinity = 10**(alpha*output_gnn.item()+beta*output_tranf.item()+gamma*abs(output_gnn.item() - output_tranf.item()))

            print(f"Predicted binding affinity for {antibody_name}-{antigen_name} pair (in IC50): {binding_affinity}")

            # Save result to a file
            with open(os.path.join("results", f"{antibody_name}_result.txt"), "w") as f:
                f.write(f"Antibody: {antibody_name}\n")
                f.write(f"Antigen: {antigen_name}\n")
                f.write(f"Predicted binding affinity (IC50): {binding_affinity}\n")

    
    os.remove(os.path.join("antibodies", file))
